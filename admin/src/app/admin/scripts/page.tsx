...
import { Plus, FileText, Trash2, Loader2, Play } from "lucide-react"
...
export default function ScriptsPage() {
  ...
  const fetchVideos = async () => {
    ...
    if (error) {
      toast.error("Failed to load script videos")
    } else {
      setVideos(data || [])
    }
    setIsLoading(false)
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this script video?")) return
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Scripts & Live Prospecting</h1>
          <p className="text-muted-foreground">
            Manage live prospecting call videos and scripts
          </p>
        </div>
      ...
      <VideoUploadDialog
        ...
        title="Upload Script Video"
        description="Add a new live prospecting call video"
