...
import '@/styles/mux-player-dark.css'
...
export default function ModernVideoPlayer({ content, type, id }: ModernVideoPlayerProps) {
  ...
  // Generate proper Mux thumbnail URLs if not provided
  const getThumbnailUrl = (video: Video) => {
    if (video.thumbnailUrl) return video.thumbnailUrl
    // Mux thumbnail API format
    return `https://image.mux.com/${video.playbackId}/thumbnail.png?width=320&height=180&fit_mode=smartcrop&time=5`
  }
    ...
    <div className="min-h-screen bg-gray-950">
      <div className="flex flex-col lg:flex-row h-screen">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="relative flex-1 bg-black [&_mux-player]:text-white">
            <style jsx global>{`
              /* Dark theme for Mux Player controls */
              mux-player::part(layer) {
                background: transparent !important;
              }
              
              mux-player::part(bottom gradient) {
                background: linear-gradient(to bottom, transparent, rgba(0, 0, 0, 0.9)) !important;
              }
              
              mux-player::part(time-range) {
                --media-range-track-background: rgba(255, 255, 255, 0.2) !important;
                --media-range-bar-color: #176FFF !important;
              }
